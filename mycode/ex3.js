...
function addProductToCart(productId) {
  // Write your code here
  let cartItem = SHOPPING_CART.find( (e) => e.id === productId)
  if(cartItem){
    cartItem.quantity += 1;
  }
  else{
...
function removeProductFromCart(productId) {
  // Write your code here
  let productIndex =  SHOPPING_CART.findIndex( (e) => e.id === productId)
  if(productIndex>=0){
    let cartItem = SHOPPING_CART[productIndex];
    cartItem.quantity >= 2 ? cartItem.quantity -= 1 : SHOPPING_CART.splice(productIndex, 1)
  }
  else{
    console('Nothing changes!')
  }
}
